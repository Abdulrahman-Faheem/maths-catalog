...
@app.route('/')
@app.route('/mathematicsCatalog')
def homePage():
    """Show the main page of the app displaying the categories with the 5 latest added items"""
    categories = session.query(Categories).all()
    items = session.query(Items).order_by(
        Items.id.desc()).limit(5)
...
@app.route('/mathematicsCatalog/<int:categories_id>')
...
@app.route('/mathematicsCatalog/<int:categories_id>/<int:items_id>')
...
def gconnect():
    ...
    150px; margin:auto; display:block; -webkit-border-radius: 150px;-moz-border-radius: 150px;"> '
...
@app.route('/mathematicsCatalog/new', methods=['GET', 'POST'])
@login_required
def newItem():
    ...
    if request.method == 'POST':  # get data from the form
        ...
        session.add(newItem)
        session.commit()
        flash("New mathematics item created!")
        return redirect(url_for('homePage'))
    else:
        return render_template('newitem.html')


@app.route('/mathematicsCatalog/<int:categories_id>/<int:items_id>/edit',
           methods=['GET', 'POST'])
@login_required
...
@app.route('/mathematicsCatalog/<int:categories_id>/<int:items_id>/delete',
           methods=['GET', 'POST'])
@login_required
...
@app.route('/mathematicsCatalog/<int:categories_id>/<int:items_id>/JSON')
